		...
		if (Common.getInstance().getAccountManager().exist(Account.BANK_PREFIX + args[0])) {
			...
			if (Tools.isValidDouble(args[1])) {
				...
				if (bankAccount.hasEnough(amount, worldName, currency.getName())) {
					...
					Common.getInstance().getServerCaller().sendMessage(sender, Common.getInstance().getLanguageManager().parse("bank_set_success", Common.getInstance().format(worldName, currency, amount), args[0]));
				} else {
