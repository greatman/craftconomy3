    ...
    public void execute(CommandSender sender, String[] args) {
        if (Common.getInstance().getAccountManager().exist(args[0], true)) {
            ...
            if (Tools.isValidDouble(args[1])) {
                ...
                if (args.length > 2) {
                    if (Common.getInstance().getCurrencyManager().getCurrency(args[2]) != null) {
                        ...
                        Common.getInstance().getServerCaller().getPlayerCaller().sendMessage(sender.getUuid(), Common
                                .getInstance()
                                .getLanguageManager().getString("currency_not_exist"));
                        return;
                    }
                }
                ...
                bankAccount.deposit(amount, worldName, currency.getName(), Cause.USER, sender.getName());
                Common.getInstance().getServerCaller().getPlayerCaller().sendMessage(sender.getUuid(), Common.getInstance().getLanguageManager().parse("bank_give_success", Common.getInstance().format(worldName, currency, amount), args[0]));
            } else {
