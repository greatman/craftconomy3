...
public class BankGiveCommand extends AbstractCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        if (Common.getInstance().getAccountManager().exist(args[0], true)) {
            ...
            if (Tools.isValidDouble(args[1])) {
                ...
                sendMessage(sender, Common.getInstance().getLanguageManager().parse("bank_give_success", Common.getInstance().format(worldName, currency, amount), args[0]));
            } else {
